    ...
    <div class="max-w-7xl mx-auto space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-3">
                <div class="flex items-center gap-1.5 bg-orange-50 border border-orange-100 rounded-full px-3 py-1.5">
                    ...
                    <span class="text-orange-700 text-xs font-black">{{ $pendingNow }} Pending</span>
                </div>
                <div class="flex items-center gap-1.5 bg-blue-50 border border-blue-100 rounded-full px-3 py-1.5">
                    ...
                    <span class="text-blue-700 text-xs font-black">{{ $processingNow }} Diproses</span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-[#0B2D5C]/05 shadow-sm p-5">
            <form method="GET" class="flex flex-wrap gap-3 items-end">
                ...
                <div class="flex gap-2">
                    ...
                    <a href="{{ route('admin.orders.index') }}" class="flex items-center gap-2 bg-slate-100 text-slate-600 text-sm font-bold px-5 py-2.5 rounded-xl hover:bg-slate-200 transition">
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-slate-50" id="ordersTableBody">
                        @foreach($orders as $order)
                            ...
                            @endphp
                            <tr class="hover:bg-slate-50/70 transition" data-order-id="{{ $order->id }}">
                                <td class="px-5 py-3.5"><span class="font-black text-[#0B2D5C] text-xs bg-[#0B2D5C]/6 px-2.5 py-1 rounded-lg">#{{ $order->order_number }}</span></td>
                                <td class="px-4 py-3.5">
                                    <div class="flex items-center gap-2">
                                        <div class="w-7 h-7 bg-gradient-to-br from-[#7AB8FF] to-[#F5A8D0] rounded-lg flex items-center justify-center text-white font-black text-[10px] flex-shrink-0">{{ strtoupper(substr($order->user->name ?? '?',0,1)) }}</div>
                                        <div>
                                            <div class="text-xs font-bold text-slate-800">{{ $order->user->name }}</div>
                                            <div class="text-[10px] text-slate-400">{{ $order->user->email }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-4 py-3.5">
                                    <span class="text-xs font-bold text-[#2D6A8F] bg-blue-50 px-2.5 py-1 rounded-lg">{{ $order->canteen->name ?? 'N/A' }}</span>
                                </td>
                                <td class="px-4 py-3.5 font-black text-slate-800 text-xs">Rp {{ number_format($order->total_amount, 0, ',', '.') }}</td>
                                <td class="px-4 py-3.5">
                                ...
                                <td class="px-4 py-3.5 text-xs text-slate-500 font-medium">{{ $order->created_at->format('d/m/Y H:i') }}</td>
        ...
        <div class="grid grid-cols-3 gap-4">
            <div class="bg-gradient-to-br from-orange-500 to-red-500 rounded-2xl p-5 text-white">
                ...
                <p class="text-3xl font-black">{{ \App\Models\Order::where('status', 'pending')->count() }}</p>
            ...
            <div class="bg-gradient-to-br from-[#2D6A8F] to-[#0B2D5C] rounded-2xl p-5 text-white">
                ...
                <p class="text-3xl font-black">{{ \App\Models\Order::where('status', 'processing')->count() }}</p>
            ...
            <div class="bg-gradient-to-br from-emerald-600 to-teal-500 rounded-2xl p-5 text-white">
                ...
                <p class="text-3xl font-black">{{ \App\Models\Order::where('status', 'completed')->count() }}</p>
