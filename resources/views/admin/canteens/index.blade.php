                ...
                <div class="bg-white rounded-3xl shadow-sm border border-slate-100 hover:shadow-md transition duration-300 overflow-hidden flex flex-col justify-between">
                    <div>
                        ...
                        <div class="h-28 bg-gradient-to-r from-blue-600 to-indigo-700 relative overflow-hidden">
                            @if($canteenImg)
                                <img src="{{ $canteenImg }}" class="w-full h-full object-cover opacity-75">
                            @endif
                            <div class="absolute inset-0 bg-black/20"></div>
                            <div class="absolute bottom-4 left-6 right-6 text-white">
                                <h2 class="text-xl font-black truncate leading-tight">{{ $canteen->name }}</h2>
                                <p class="text-white/80 text-[10px] font-bold uppercase tracking-wider mt-0.5">
                                    <i class="fa-solid fa-location-dot mr-1"></i>{{ $canteen->location ?? 'Telkom University' }}
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="p-6 space-y-4">
                            ...
                            <div class="flex items-center gap-3.5 pb-4 border-b border-slate-100">
                                @if($canteen->ibuKantin && $canteen->ibuKantin->avatar)
                                    <img src="{{ asset('storage/avatars/' . $canteen->ibuKantin->avatar) }}" 
                                ...
                                <div class="overflow-hidden">
                                    ...
                                    <p class="text-sm font-bold text-slate-800 truncate">{{ $canteen->ibuKantin->name ?? 'N/A' }}</p>
                                    <p class="text-xs text-slate-500 truncate">{{ $canteen->ibuKantin->email ?? 'N/A' }}</p>
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-3 gap-3">
                                <div class="text-center p-2.5 bg-slate-50 rounded-2xl">
                                    <div class="text-xl font-black text-blue-600">{{ $canteen->orders_count }}</div>
                                    <div class="text-[9px] font-bold text-slate-500 uppercase tracking-wide">Pesanan</div>
                                </div>
                                <div class="text-center p-2.5 bg-slate-50 rounded-2xl">
                                    <div class="text-xl font-black text-green-600">{{ $canteen->menus_count }}</div>
                                    <div class="text-[9px] font-bold text-slate-500 uppercase tracking-wide">Menu</div>
                                </div>
                                <div class="text-center p-2.5 bg-slate-50 rounded-2xl">
                                    <div class="text-xl font-black text-purple-600">{{ $canteen->vouchers_count }}</div>
                    ...
                    <div class="px-6 py-4 border-t border-slate-100 flex items-center justify-between bg-slate-50/50 rounded-b-3xl">
                        ...
                        <div class="flex gap-4">
                            <button onclick="openEditCanteenModal(
                                {{ $canteen->id }}, 
                                '{{ addslashes($canteen->name) }}',
                                '{{ addslashes($canteen->location) }}',
                                '{{ addslashes($canteen->ibuKantin->name ?? '') }}',
                                '{{ addslashes($canteen->ibuKantin->username ?? '') }}',
                                '{{ addslashes($canteen->ibuKantin->email ?? '') }}',
                                '{{ addslashes($canteen->ibuKantin->phone ?? '') }}'
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mt-12">
            <div class="bg-white rounded-3xl shadow-sm p-6 border border-slate-100 flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-2xl font-black text-slate-800">{{ $canteens->count() }}</p>
                </div>
            </div>
            <div class="bg-white rounded-3xl shadow-sm p-6 border border-slate-100 flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-2xl font-black text-slate-800">{{ $canteens->sum('menus_count') }}</p>
                </div>
            </div>
            <div class="bg-white rounded-3xl shadow-sm p-6 border border-slate-100 flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-2xl font-black text-slate-800">{{ $canteens->sum('vouchers_count') }}</p>
                </div>
            </div>
            <div class="bg-white rounded-3xl shadow-sm p-6 border border-slate-100 flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-2xl font-black text-slate-800">{{ $canteens->sum('orders_count') }}</p>
                </div>
            </div>
        </div>
                ...
                <tbody class="divide-y divide-slate-100">
                    @foreach($canteens as $canteen)
                        <tr class="hover:bg-slate-50/50 transition">
                            <td class="px-6 py-4 font-bold text-slate-800">{{ $canteen->name }}</td>
                            <td class="px-6 py-4">
                                <div class="text-sm font-bold text-slate-800">{{ $canteen->ibuKantin->name ?? '-' }}</div>
                                <div class="text-xs text-slate-500">{{ $canteen->ibuKantin->email ?? '-' }}</div>
                            </td>
                            <td class="px-6 py-4 text-center"><span class="inline-block px-3 py-1 bg-blue-50 text-blue-700 rounded-full text-xs font-bold">{{ $canteen->orders_count }}</span></td>
                            <td class="px-6 py-4 text-center"><span class="inline-block px-3 py-1 bg-green-50 text-green-700 rounded-full text-xs font-bold">{{ $canteen->menus_count }}</span></td>
                            <td class="px-6 py-4 text-center"><span class="inline-block px-3 py-1 bg-purple-50 text-purple-700 rounded-full text-xs font-bold">{{ $canteen->vouchers_count }}</span></td>
...
<div id="addCanteenModal" class="fixed inset-0 z-[100] hidden items-center justify-center bg-black/60 backdrop-blur-sm p-4 overflow-y-auto">
    <div class="bg-white rounded-[30px] shadow-2xl w-full max-w-lg my-8">
        ...
        <form action="{{ route('admin.canteens.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-4 max-h-[75vh] overflow-y-auto">
            @csrf
