        ...
        <div id="pending" class="tab-content p-8 active">
            @forelse($pendingOrders as $order)
                ...
                <div class="order-card bg-gradient-to-r from-orange-50 to-yellow-50 rounded-lg p-6 mb-4 border-l-4 border-orange-500 shadow-sm hover:shadow-md transition">
                    ...
                    {{-- ===== LOKASI PENGAMBILAN — paling atas, paling mencolok ===== --}}
                    @if($lokasiMatch)
                        <div class="mb-4 rounded-xl px-4 py-3 flex items-center gap-3 font-bold
                            {{ $isAntar ? 'bg-blue-100 border border-blue-300 text-blue-800' : 'bg-green-100 border border-green-300 text-green-800' }}">
                            <span class="text-2xl">{{ $isAntar ? '🛵' : '🏃' }}</span>
                            <div>
                                <p class="text-xs uppercase tracking-widest opacity-70 mb-0.5">{{ $isAntar ? 'ANTAR KE LOKASI' : 'AMBIL SENDIRI DI KANTIN' }}</p>
                                <p class="text-sm">{{ $lokasiMatch }}</p>
                            </div>
                        </div>
                    @else
