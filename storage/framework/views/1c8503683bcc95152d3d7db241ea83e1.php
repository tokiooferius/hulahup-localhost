...
<?php $__env->startSection('extra-css'); ?>
        ...
        .cart-item-anim { animation: bounce-in 0.3s ease-out forwards; }
...
<aside id="cartSidebar" class="fixed right-0 top-0 h-full w-[340px] bg-white shadow-2xl z-[60] transition-transform duration-500 translate-x-full border-l border-slate-100 flex flex-col" style="max-width: 90vw;">

    <!-- Header -->
    <div class="flex justify-between items-center px-5 py-4 border-b border-slate-100 shrink-0">
        <div class="flex items-center gap-2">
            <i class="fa-solid fa-basket-shopping text-[#5B88B2]"></i>
            <h3 class="text-base font-bold text-[#122C4F]">Keranjang Saya</h3>
            <span id="cartCount" class="bg-[#122C4F] text-white px-2 py-0.5 rounded-full text-[10px] font-black">0</span>
        </div>
        <button onclick="toggleCart()" class="text-slate-400 hover:text-red-500 w-7 h-7 flex items-center justify-center rounded-full hover:bg-red-50 transition text-sm">
            <i class="fa-solid fa-xmark"></i>
        </button>
    </div>

    <!-- Items Scrollable Area -->
    <div id="cartItems" class="flex-1 overflow-y-auto no-scrollbar px-4 py-3 space-y-2" style="min-height: 0;">
        <div class="text-center py-16 opacity-20">
            <i class="fa-solid fa-basket-shopping text-5xl mb-3 block"></i>
            <p class="text-xs font-bold uppercase tracking-widest">Belum ada pesanan</p>
        </div>
    </div>

    <!-- Bottom Fixed Section -->
    <div class="border-t-2 border-dashed border-slate-100 px-5 py-4 shrink-0 space-y-3 bg-white">
        
        <!-- Subtotal & Total -->
        <div class="space-y-1.5">
            <div class="flex justify-between text-[11px] text-slate-400 font-bold uppercase tracking-wider">
                <span>Subtotal</span>
                <span id="subtotal">RP 0</span>
            </div>
            <div class="flex justify-between items-center pt-2 border-t border-slate-100">
                <span class="text-sm font-black text-[#122C4F] italic">Total</span>
                <span id="totalPrice" class="text-xl font-black text-[#5B88B2] tracking-tighter">RP 0</span>
            </div>
        </div>

        <!-- Lokasi Pengambilan (compact) -->
        <div class="bg-blue-50 rounded-xl p-3 border border-blue-100">
            <p class="font-bold text-[#122C4F] mb-2 flex items-center gap-1.5 text-[11px] uppercase tracking-wide">
                <i class="fas fa-map-marker-alt text-blue-500 text-xs"></i> Lokasi Pengambilan
            </p>
            <div class="flex gap-1.5 mb-2">
                <button type="button" id="btnPickup"
                    onclick="setDeliveryType('pickup')"
                    class="flex-1 py-1.5 text-[11px] font-bold rounded-lg bg-[#122C4F] text-white transition">
                    🏃 Ambil Sendiri
                </button>
                <button type="button" id="btnDeliver"
                    onclick="setDeliveryType('deliver')"
                    class="flex-1 py-1.5 text-[11px] font-bold rounded-lg bg-white border border-slate-200 text-slate-600 transition hover:border-blue-400">
                    🛵 Antar Lokasi
                </button>
            </div>
            <div id="pickupMsg" class="text-[10px] text-green-700 bg-green-50 border border-green-200 rounded-lg px-2.5 py-1.5 font-medium leading-snug">
                ✅ Ambil di kantin — pengelola kantin akan siapkan pesanan kamu
            </div>
            <div id="deliverForm" class="hidden space-y-1.5 mt-1">
                <select id="pickupArea" class="w-full text-[11px] px-2.5 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-300 outline-none bg-white">
                ...
                <input type="text" id="pickupDetail"
                    placeholder="Detail lokasi: Ruang 301, meja kiri..."
                    class="w-full text-[11px] px-2.5 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-300 outline-none bg-white">
            </div>
        </div>

        <!-- Checkout Button -->
        <button onclick="openPaymentModal()" class="w-full bg-[#5B88B2] text-white py-3.5 rounded-2xl font-black text-xs shadow-lg hover:bg-[#122C4F] transition-all uppercase tracking-widest active:scale-95">
        ...
        <div class="bg-white w-full max-w-md rounded-[35px] shadow-2xl p-8 animate-in zoom-in duration-300">
            ...
            <div class="flex justify-between items-center mb-6">
                <div class="flex items-center gap-2">
                    ...
                    <h3 class="text-xl font-black text-[#122C4F]">Pakai Voucher</h3>
                </div>
                <button onclick="closeVoucherModal()" class="text-slate-400 hover:text-red-500 w-8 h-8 flex items-center justify-center rounded-full hover:bg-red-50 transition">
                    <i class="fa-solid fa-xmark"></i>
                </button>
            </div>
            ...
            <div class="mb-5">
                ...
                <div class="flex gap-2">
                    <input type="text" id="voucherCodeInput"
                        placeholder="contoh: TELU2026"
                        class="flex-1 px-4 py-3 border-2 border-slate-200 rounded-2xl focus:outline-none focus:border-purple-400 font-mono text-sm uppercase transition"
                        oninput="this.value = this.value.toUpperCase()"
                        onkeydown="if(event.key==='Enter') checkVoucherCode()">
                    <button onclick="checkVoucherCode()" id="voucherCheckBtn"
                        class="bg-purple-600 hover:bg-purple-700 text-white font-bold px-5 py-3 rounded-2xl transition text-sm whitespace-nowrap">
                        Cek Kode
                    </button>
                </div>
                <!-- Feedback message -->
                <div id="voucherFeedback" class="hidden mt-2 text-sm font-medium rounded-xl px-4 py-2.5"></div>
            </div>

            <!-- Divider -->
            <div class="flex items-center gap-3 mb-4">
                <div class="flex-1 border-t border-slate-100"></div>
                <span class="text-xs text-slate-400 font-bold uppercase">atau pilih promo tersedia</span>
                <div class="flex-1 border-t border-slate-100"></div>
            </div>

            <!-- Daftar Voucher Default -->
            <div id="modalVoucherList" class="space-y-3">
            </div>

            <!-- Voucher Aktif sekarang -->
            <div id="activeVoucherInfo" class="hidden mt-4 bg-green-50 border border-green-200 rounded-2xl p-4">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-xs text-green-600 font-bold uppercase tracking-wide">✅ Voucher Aktif</p>
                        <p id="activeVoucherName" class="font-black text-[#122C4F] text-sm mt-0.5"></p>
                    </div>
                    <button onclick="removeVoucher()" class="text-red-400 hover:text-red-600 text-xs font-bold transition flex items-center gap-1">
                        <i class="fas fa-times"></i> Hapus
                    </button>
                </div>
            </div>

            <button onclick="closeVoucherModal()" class="w-full mt-5 text-slate-400 hover:text-slate-600 text-xs font-bold uppercase tracking-widest py-3 transition">
                Tutup
            </button>
        </div>
    </div>
    ...
    <script>
        ...
        let cart = [];
        try {
            const savedCart = localStorage.getItem('foodtyu_cart');
            if (savedCart) {
                cart = JSON.parse(savedCart);
                // Migrasi: pastikan semua item punya field baru
                cart = cart.map(item => ({
                    ...item,
        ...
        function addToCart(name, price, img, canteenId, description = '', canteenName = '') {
            const absoluteImg = getAbsoluteUrl(img);
            const existingItem = cart.find(item => item.name === name && item.canteen_id === canteenId);
            if (existingItem) {
                existingItem.qty += 1;
            } else {
                cart.push({ name, price, img: absoluteImg, canteen_id: canteenId, qty: 1, description: description, canteen_name: canteenName });
            }
            updateCartUI();
        }
        ...
        function updateCartUI() {
            ...
            if (cart.length === 0) {
                cartItems.innerHTML = `
                    <div class="text-center py-14 opacity-20">
                        <i class="fa-solid fa-basket-shopping text-5xl mb-3 block"></i>
                        <p class="text-xs font-bold uppercase tracking-widest">Belum ada pesanan</p>
                    </div>`;
            } else {
            ...
            cart.forEach((item, index) => {
                ...
                const kantin = item.canteen_name ? `<span class="inline-flex items-center gap-1 text-[9px] font-bold text-[#5B88B2] bg-blue-50 px-1.5 py-0.5 rounded-full"><i class="fa-solid fa-store" style="font-size:7px"></i> ${item.canteen_name}</span>` : '';
                cartItems.innerHTML += `
                    <div class="group cart-item-anim border border-slate-100 rounded-xl p-2.5 hover:border-[#5B88B2]/40 hover:bg-slate-50/50 transition-all">
                        <div class="flex items-start gap-2.5">
                            <!-- Foto -->
                            <div class="w-14 h-14 bg-slate-100 rounded-lg overflow-hidden shrink-0 shadow-sm">
                                <img src="${item.img}" alt="${item.name}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" onerror="this.src='https://via.placeholder.com/56'">
                            </div>
                            <!-- Info -->
                            <div class="flex-1 min-w-0">
                                ${kantin}
                                <p class="text-[11px] font-bold text-[#122C4F] leading-tight line-clamp-1 mt-0.5">${item.name}</p>
                                ${desc ? `<p class="text-[9px] text-slate-400 mt-0.5 line-clamp-1 leading-snug">${desc}</p>` : ''}
                                <div class="flex items-center justify-between mt-1.5">
                                    <p class="text-[11px] text-[#5B88B2] font-black">Rp ${item.price.toLocaleString('id-ID')}</p>
                                    <span class="text-[9px] font-bold text-slate-400">= Rp ${itemTotal.toLocaleString('id-ID')}</span>
                                </div>
                            </div>
                        </div>
                        <!-- Qty Controls -->
                        <div class="flex items-center justify-between mt-2 pt-2 border-t border-slate-100">
                            <div class="flex items-center gap-1">
                                <button onclick="changeQty(${index}, -1)" class="w-6 h-6 rounded-md bg-slate-100 hover:bg-red-50 hover:text-red-500 text-slate-500 flex items-center justify-center transition-all text-[9px]">
                                    <i class="fa-solid fa-minus"></i>
                                </button>
                                <span class="w-7 text-center text-xs font-black text-[#122C4F]">${item.qty}</span>
                                <button onclick="changeQty(${index}, 1)" class="w-6 h-6 rounded-md bg-[#122C4F]/10 hover:bg-[#122C4F] hover:text-white text-[#122C4F] flex items-center justify-center transition-all text-[9px]">
                                    <i class="fa-solid fa-plus"></i>
                                </button>
                            </div>
                            <button onclick="changeQty(${index}, -${item.qty})" class="text-[9px] text-red-400 hover:text-red-600 font-bold transition-colors flex items-center gap-1">
            ...
            // Update count badge (angka saja, bukan "X Items")
            const totalQtyAll = cart.reduce((s, i) => s + (i.qty || 1), 0);
            cartCount.innerText = totalQtyAll > 0 ? totalQtyAll : '0';
        ...
        // Daftar voucher yang tersedia (hanya 1 voucher default/bawaan)
        const allVouchers = [
            { id: 'NEWUSER5', name: 'Diskon Pengguna Baru 5%', discount: 5, category: 'all' }
        ];

        // Fungsi untuk mendapatkan voucher
        function getDailyVouchers() {
            return allVouchers;
        }

        // Tampilkan daftar voucher di hero section (jika elemen ada)
        ...
        function displayDailyVouchersInModal() {
            ...
            // Update informasi voucher aktif di modal
            updateActiveVoucherInfoInModal();
        }
        ...
        // Fungsi untuk memeriksa kode voucher (custom input)
        function checkVoucherCode() {
            const input = document.getElementById('voucherCodeInput');
            const btn = document.getElementById('voucherCheckBtn');
            
            if (!input || !btn) return;
            
            const code = input.value.trim().toUpperCase();
            if (!code) {
                showVoucherFeedback('Silakan masukkan kode voucher.', 'error');
                return;
            }
            ...
            .then(data => {
                ...
                if (data.valid) {
                    selectVoucher(data.code, data.discount_percentage, data.description || `Diskon ${data.discount_percentage}% dari kode ${data.code}`);
                    showVoucherFeedback(`Voucher "${data.code}" berhasil digunakan!`, 'success');
                } else {
                    showVoucherFeedback(data.message || 'Kode voucher tidak valid.', 'error');
                }
            })
            .catch(err => {
                btn.disabled = false;
                btn.innerHTML = originalText;
                showVoucherFeedback('Gagal memproses voucher. Silakan coba lagi.', 'error');
            });
        }
        ...
        function showVoucherFeedback(msg, type) {
            ...
            feedback.className = 'mt-2 text-sm font-medium rounded-xl px-4 py-2.5';
        ...
        // Fungsi untuk menghapus voucher aktif
        function removeVoucher() {
            sessionStorage.removeItem('voucher_active');
            sessionStorage.removeItem('active_voucher_id');
        ...
        // Fungsi untuk meng-update info voucher aktif di modal
        function updateActiveVoucherInfoInModal() {
            const infoDiv = document.getElementById('activeVoucherInfo');
            const nameEl = document.getElementById('activeVoucherName');
            
            if (!infoDiv || !nameEl) return;
            
            if (isVoucherActive()) {
                const voucherName = sessionStorage.getItem('voucher_name');
                const discount = getDiscountPercent();
                nameEl.textContent = `${voucherName} (Diskon ${discount}%)`;
        ...
        function processFinalPayment() {
            ...
            // Terapkan diskon voucher ke masing-masing kantin secara proporsional
            let calculatedGrandTotal = 0;
            const keys = Object.keys(cartByCanteen);
            keys.forEach((cid, index) => {
                const subtotal = cartByCanteen[cid].total;
                if (index === keys.length - 1) {
                    // Canteen terakhir mendapatkan pembulatan sisa totalPaid
                    cartByCanteen[cid].total = totalPaid - calculatedGrandTotal;
                } else {
                    const discountValue = Math.round(subtotal * discountPercent / 100);
                    cartByCanteen[cid].total = subtotal - discountValue;
                    calculatedGrandTotal += cartByCanteen[cid].total;
                }
            });
        ...
        // Render cart items if any exist from localStorage on load
        if (cart.length > 0) {
            updateCartUI();
        }
        ...
        function showMenuDetailFromHome(menu) {
            ...
            const absoluteImg = getAbsoluteUrl(menu.image_url);
                ...
                <div class="bg-white rounded-3xl shadow-2xl max-w-md w-full overflow-auto">
                    ...
                    <div class="p-6">
                        <div class="mb-4 h-48 bg-slate-200 rounded-xl overflow-hidden">
                            <img src="${absoluteImg}" alt="${menu.name}" class="w-full h-full object-cover">
                        </div>
                        ...
                        <div class="flex gap-2">
                            ...
                            <button onclick="addToCart('${menu.name}', ${menu.price}, '${absoluteImg}', ${menu.canteen_id}, '${(menu.description || '').replace(/'/g, '\'\'')}', '${(menu.canteen?.name || 'Kantin').replace(/'/g, '\'')}'); document.getElementById('homeMenuDetailModal').remove(); toggleCart();" 
